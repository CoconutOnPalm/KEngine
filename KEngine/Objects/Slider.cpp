...
namespace ke
{
	...
	void Slider::setOrigin(int origin)
	{
		...
		{
			origin = RIGHT_TOP;
			throw_error("Slider::setOrigin(..)", "Given object origin is incorrect", "ERROR");
		}
	...
	float Slider::update(const sf::Vector2f& mousePosition, sf::Event& event, sf::Mouse::Button button, sf::View* view)
	{
		...
		float st_top_pos_buff = m_slider_track.getShapeCenter().y - st_size_buff * 0.5; // slider track top


		// position guard
		if (view)
			m_slider.setPosition(sf::Vector2f(m_slider.getPosition().x, st_top_pos_buff + s_size_buff * 0.5 + ((view->getCenter().y - m_top_pos) - view->getSize().y * 0.5) * (st_size_buff - s_size_buff) / ((m_field_height - view->getSize().y))));
