...
namespace ke
{
	...
	bool Button::isInvaded(const sf::Vector2f& mousePosition) const
	{
		...
		if (m_texture_shape.getRotation() == 0.0f)
			return abs(m_shapeCenter.x - mousePosition.x) < m_texture_shape.getSize().x * m_texture_shape.getScale().x * 0.5 && abs(m_shapeCenter.y - mousePosition.y) < m_texture_shape.getSize().y * m_texture_shape.getScale().y * 0.5;

		// rotated rectangle - point collision detection
		// works only when m_origin is set to MIDDLE_MIDDLE
		return
			abs((cos(DTR(-m_texture_shape.getRotation())) * m_shapeCenter.x - sin(DTR(-m_texture_shape.getRotation())) * m_shapeCenter.y) - (cos(DTR(-m_texture_shape.getRotation())) * mousePosition.x - sin(DTR(-m_texture_shape.getRotation())) * mousePosition.y)) < m_texture_shape.getSize().x * m_texture_shape.getScale().x * 0.5 &&
			abs((cos(DTR(-m_texture_shape.getRotation())) * m_shapeCenter.y + sin(DTR(-m_texture_shape.getRotation())) * m_shapeCenter.x) - (cos(DTR(-m_texture_shape.getRotation())) * mousePosition.y + sin(DTR(-m_texture_shape.getRotation())) * mousePosition.x)) < m_texture_shape.getSize().y * m_texture_shape.getScale().y * 0.5;
	}
